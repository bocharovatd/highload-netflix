...
#### Описание
**Titus-isolate** [^35] — это алгоритмическая система Netflix для изоляции CPU-ресурсов контейнеров, устраняющая проблему ***collation noise***.
...
- ***titus-isolate*** - процесс пользовательского пространства (запускает три события, влекущих оптимизацию размещения: добавление нового контейнера, удаление завершившегося контейнера, перебалансировка)
- алгоритм 
    - при вызове события размещения titus-isolate вызывает службу оптимизации (тоже запущенную как titus-isolate процесс)
    ...
    - прогнозирование является NP-трудной задачей, поэтому есть ограничения на время принятия решения
