...
### Резервирование компонетов 
| **Компонент** | **Способ резервирования** | **Описание** |
| --- | --- | --- |
| **PostgreSQL** | - Master-slave репликация<br> - Patroni для автоматического failover<br> - Индексация в ElasticSearch через Delta (Kafka Keystone)<br> - EVCache для кэширования частых запросов | Чтение распределяется между репликами. Полусинхронная репликация (подтверждение записи хотя бы на одном узле). Индексы синхронизируются с ElasticSearch для ускорения поиска и снижения нагрузки на PostgreSQL |
| **Cassandra** | - Межрегиональная репликация (LOCAL_QUORUM)<br> - EVCache для кэширования частых запросов | Запись подтверждается большинством узлов в локальном кластере. Кэширование снижает нагрузку на БД. |
| **ElasticSearch** | - Реплики для каждого шарда<br> - Асинхронная реплиликация в разных ДЦ<br> - Snapshot-бэкапы в S3 | Каждый шард имеет 2 реплики. Бэкапы позволяют восстановливать индексы. |
| **Tarantool (In-memory)** | - Master-master репликация<br> - EVCache для кэширования частых запросов  | Полусинхронная репликация гарантирует доступность. EVCache кэширует сессии пользователей. |
| **S3** | - Multi-AZ + версионирование объектов | Данные автоматически реплицируются между AZ. Версионирование сохраняет историю изменений (например, предыдущие версии медиафайлов). |
| **Микросервисы**<br> (ex: NMDB) | - Кластеризация, health checks, autoscaling через Kubernetes (Titus)<br> - EVCache для кэширования частых запросов  | Контейнеры перезапускаются при сбоях. EVCache кэширует метаданные медиа (UUID, URL). |
| **Apache Kafka** | - Репликация партиций (min.insync.replicas=2)<br>- MirrorMaker 2.0 для межрегиональной синхронизации | Данные дублируются на 3+ брокерах. MirrorMaker синхронизирует данные между ЦОД. |
| **Hadoop** | - Резервные кластеры в Amazon EMR<br> - Autoscaling на базе Amazon EMR| При сбое основного кластера активируется резервный. Автоскейлинг добавляет узлы при пиковой нагрузке (например, анализ истории просмотров). |
