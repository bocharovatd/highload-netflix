...
| **Cassandra** | - Межрегиональная репликация (LOCAL_QUORUM)<br> - EVCache для кэширования частых запросов<br> - Резервное копирование через snapshot-бэкапы в S3 + incremental backups | **Межрегиональная репликация**: данные копируются в 3+ ЦОД. **LOCAL_QUORUM**: запись подтверждается большинством узлов в локальном кластере. **EVCache**: кэширует метаданные медиа. **Snapshot-бэкапы**: полные копии данных в S3. **Incremental backups**: сохраняются только изменения с момента последнего снапшота. |
...
| **Tarantool (In-memory)** | - Master-master репликация<br> - EVCache для кэширования частых запросов | **Master-master репликация**: запись возможна в любой узел, данные синхронизируются между всеми инстансами, полусинхронная репликая гарантирует доступность **EVCache**: кэширует сессии пользователей и токены аутентификации. |
...
| **Микросервисы**<br> (ex: NMDB) | - Кластеризация, health checks, autoscaling через Kubernetes (Titus)<br> - EVCache для кэширования частых запросов<br> - Circuit Breaker через Hystrix | **Кластеризация**: сервисы развертываются в 3+ экземплярах. **Health Checks**: Eureka проверяет доступность каждые 30 сек. **Autoscaling**: кластер расширяется при пиковой нагрузке. **EVCache**: кэширует UUID медиафайлов и URL. **Hystrix**: блокирует запросы к упавшим сервисам на 30 сек. |
