...
| Таблица | Описание |
| --- | --- |
| `user` | Хранит данные пользователей: логин, email, хэш пароля, тип подписки (basic, standard, premium), дата регистрации. Используется для аутентификации, управления подписками и персонализации. |
| `favorite` | Хранит избранный контент пользователей. Связывает пользователя с контентом, который он добавил в "Мое избранное". |
| `watch_history` | Хранит историю просмотров: пользователь, медиа, прогресс просмотра (в минутах), дата просмотра. |
| `recommendation` | Хранит рекомендации для пользователей: векторные данные (например, embeddings), дата создания. Используется для персонализации контента. |
| `notification` | Хранит уведомления для пользователей: текст сообщения, статус доставки (pending, sent, failed), дата отправки. Используется для информирования о новых сериях, акциях и т.д. |
| `content` | Основная таблица контента: тип (фильм/сериал), возрастной рейтинг, рейтинг контента. |
| `movie` | Хранит данные о фильмах: название, дата выхода, тип (фильм/сериал). Основная информация для отображения на платформе. Связана с таблицей контента. |
| `series` | Хранит данные о сериалах: название, количество сезонов, эпизодов, даты выхода (начало и конец). |
| `episode` | Хранит данные об эпизодах сериалов: связь с сериалом, номер сезона, номер эпизода. Используется для организации структуры сериалов. |
| `trailer` | Хранит данные о трейлерах: связь с контентом, длительность, дата выхода. Используется для продвижения контента. |
| `media` | Хранит медиа-объекты: тип (фильм, эпизод, трейлер), длительность. Используется для связи с таблицей медиафайлов, а также в истории просмотров. |
| `media_files` | Предоставляет доступ к медиа файлам: качество (Low, Medium, SD, HD, Ultra HD), ссылка на S3. Используется для хранения и доставки медиафайлов. |
| `genre` | Хранит список жанров (например, комедия, драма, фантастика). Используется для категоризации контента. |
| `content_actors` | Связывает контент с актерами (многие-ко-многим). Используется для отображения актеров, участвующих в фильмах/сериалах. |
| `actor` | Хранит данные об актерах: имя, фото. Используется для отображения информации об актерах. |
| `content_genre` | Связывает контент с жанрами (многие-ко-многим). Используется для классификации контента по жанрам. |
...
| ENUM | 4 байта |
| DECIMAL(3,1) | 5 байт |
| INT, TIMESTAMP, DATE | 4 байт |
| TEXT | 100–500 байт (среднее значение) |
| JSONB | 30_720 байт (в данном случае) |

Для расчета понадобятся уже рассчитанные данные:
- зарегистрированных пользователей - 2B
- фильмов - 13_650
- сериалов - 5_850
- серий - 93_600
- жанров - 28 [^27]
- количество историй просмотров - 3310
- количество позиций в избранном - 50

| Таблица | Рассчет размера | Размер строки | Всего строк | Размер таблицы |
| --- | --- | --- | --- | --- |
| user | 16 (id) + 50 (login) + 50 (email) + 100 (password_hash) + 4 (subscription_plan) + 4 (created_at) | 224 байт | 2B | 417 Гб |
| content | 16 (id) + 4 (content_type) + 5 (age_rating) + 5 (rating) | 30 байт | 19_500 (13_650 фильмов + 5_850 сериалов) | 571 Кб |
| media | 16 (id) + 4 (media_type) + 4 (duration) | 24 байта | 126_750 (13_650 фильмов + 93_600 эпизодов + 19_500 трейлеров) | 2.9 Мб |
| movie | 16 (id) + 16 (content_id) + 16 (media_id) + 100 (title) + 4 (release_date) | 152 байт | 13_650 | 1.98 Мб |
| series | 16 (id) + 16 (content_id) + 100 (title) + 4 (total_seasons) + 16 (episodes_per_season) + 4 (start_date) + 4 (end_date) | 160 байт | 5_850 | 914 Кб |
| episode | 16 (id) + 16 (media_id) + 16 (series_id) | 48 байт | 93_600 | 4.28 Мб |
| trailer | 16 (id) + 16 (content_id) + 16 (media_id) + 500 (description) | 548 байт | 19_500 | 10.19 Мб |
| media_files | 16 (id) + 16 (media_id) + 4 (quality) + 500 (s3_url) | 536 байт | 633_750 (126_750 × 5) | 324 Мб |
...
| favorite | 16 (user_id) + 16 (content_id) + 4 (created_at) | 36 байт | 20B (2B * 10) | 671 Гб |
| watch_history | 16 (user_id) + 16 (media_id) + 4 (watched_at) + 4 (progress) | 40 байт | 6 620B (2B * 3310) | 240 Тб |
| recommendation | 16 (user_id) + 16 (content_id) + 30_720 (vector_data) + 4 (created_at) | 30_756 байт | 2B | 56 Тб |
| notification | 16 (id) + 16 (user_id) + 100 (message) + 4 (delivery_status) + 4 (sent_at) | 140 байт | 3B (2B × 1.5) | 391 Гб |

Итого: 417 Гб + 0_000544 Гб + 0_002832 Гб + 0_00198 Гб + 0_000871 Гб + 0_00418 Гб + 0_00995 Гб + 0_316 Гб + 0_00581 Гб + 0_00175 Гб + 0_00000172 Гб + 0_0323 Гб + 671 Гб + 245 760 Гб + 57 344 Гб + 391 Гб = 304 583_38 Гб = 297_44 Тб

### Рассчет нагрузки на чтение и запись

- чтение из таблицы user - 91.8M / 86_400 = 1063, запись - 1 (посчитанный RPS на регистрацию)
- пользователь смотрит контент 2 часа в день, средняя продолжительность контента - 0.85 * 45 + 0.15 * 90 = 51.75 минут, то есть 120 / 51.75 = 2.32 запроса в день на пользователя. Тогда обращение к таблице серий и сериалов - 0.85 * 91.8M * 2.32 / 86_400 = 2095, к таблице фильмов - 0.15 * 91.8M * 2.32 / 86_400 = 370. Также необходимо учесть обращение к таблицам при поиске, что составляет 0.85 * 2656 = 2258 - для сериалов,  0.15 * 2656 = 397 для фильмов. Итого RPS на чтение из таблицы сериалов - 2095 + 2258 = 4353, серий - 2095, фильмов - 370 + 397 = 767
- чтение из таблицы trailer - 1487 (посчитанный RPS на просмотр трейлера)
- чтение из таблицы media - суммарный RPS чтения серий, фильмов при просмотре (без учета обращений при поиске) и трейлеров - 2095 + 370 + 1487 = 3952
- чтение из таблицы media-files - RPS на чтение из медиа - 3952
- чтение из таблицы content - суммарный RPS чтения сериалов и фильмов - 4353 + 767 = 5120
- чтение из таблицы actor - 7438 (посчитанный RPS просмотра страницы фильма или сериала)
- чтение из таблицы watch_history - 903 (посчитанный RPS на обращение к истории просмотров), запись - RPS на чтение медиа - 3952
