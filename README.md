...
| **`media`** | Техническая сущность: тип медиа (movie, episode, trailer). Служит для связи с DASH-сегментами через дорожки (track) и компоненты (component). |
...
| --- | --- | --- | --- | --- |
| user | 16 (id) + 50 (login) + 50 (email) + 100 (password_hash) + 4 (subscription_plan) + 4 (created_at) + 4 (updated_at) | 228 байт | 2B | 456 Гб |
| poster | 16 (id) + 16 (content_id) + 500 (poster_url) + 5 (age_limit) + 5 (rating) | 542 байт | 19_500 | 10.1 Мб |
| content | 16 (id) + 4 (content_type) + 100 (title) + 4 (release_date) + 30 (country) + 500 (description) | 654 байт | 19_500 | 12.4 Мб |
...
| event | 16 (id) + 16 (component_id) + 4 (start_time) + 4 (end_time) + 500 (s3_url) + 4 (sequence_num) | 544 байт | 61_863_750 | 32.1 Тб |
| movie | 16 (id) + 16 (content_id) + 16 (media_id) | 48 байт | 13_650 | 0.64 Мб |
| series | 16 (id) + 16 (content_id) + 4 (total_seasons) | 36 байт | 5_850 | 0.21 Мб |
| episode | 16 (id) + 16 (series_id) + 16 (media_id) + 2 (season_number) + 2 (episode_number) + 500 (description) | 552 байт | 93_600 | 49.4 Мб |
| trailer | 16 (id) + 16 (content_id) + 16 (media_id) | 48 байт | 19_500 | 0.91 Мб |
...
| actor | 16 (id) + 100 (name) + 500 (photo_url) | 616 байт | 56_438 | 33.1 Мб |
| favorite | 16 (user_id) + 16 (content_id) + 4 (created_at) + 4 (updated_at) | 40 байт | 20B | 745 Гб |
| watch_history | 16 (user_id) + 16 (media_id) + 4 (watched_at) + 4 (progress) | 40 байт | 6_620B (2B * 3310) | 247 Тб |
| recommendation | 16 (user_id) + 30_720 (vector_data) + 4 (created_at) + 4 (updated_at) | 30_744 байт | 2B | 57.3 Тб |
| notification | 16 (id) + 16 (user_id) + 100 (message) + 4 (delivery_status) + 4 (sent_at) | 140 байт | 3B (2B × 1.5) | 391 Гб |


**Итого: 456 Гб (user) + 0.01 Гб (poster) + 0.012 Гб (content) + 0.002 Гб (media) + 0.008 Гб (track) + 0.021 Гб (component) + 0.01 Гб (video_component) + 0.013 Гб (audio_component) + 32_100 Гб (event) + 0.0006 Гб (movie) + 0.0002 Гб (series) + 0.049 Гб (episode) + 0.0009 Гб (trailer) + 0.005 Гб (content_actors) + 0.0017 Гб (content_genre) + 0.0000017 Гб (genre) + 0.033 Гб (actor) + 745 Гб (favorite) + 247_000 Гб (watch_history) + 57_300 Гб (recommendation) + 391 Гб (notification) = 305_105 Гб ≈ 297.9 Тб**
...
### Денормализация
- добавление информации о постера в таблицу content
- добавление JSONB полей actors, genres в таблицу content
- добавление массива с количеством серий в каждом сезоне в таблицу series
- добавление поля contend_id, соответсвующего сериал.content_id, в таблицу episodes для удобства шардирования по content_id
...
| poster | content_id, age_limit, rating | Быстрый доступ к постерам по контенту, фильтрация по рейтингу и возрастным ограничениям. |
| content | title, release_date, genres , actors | Поиск по названию, дате выхода, жанрам и актерам. |
| track | media_id, track_type | Поиск дорожек по медиа и типу (аудио/видео). |
| component | track_id, resolution, language | Оптимизация запросов к параметрам компонентов (разрешение, язык). |
| event | component_id, sequence_num | Быстрый доступ к сегментам по компоненту и порядку воспроизведения. |
| episode | series_id, episode_number | Поиск эпизодов по сериалу и навигация по эпизодам внутри сезонов. |
| trailer | content_id, media_id | Связь трейлеров с контентом и медиа. |
| content_actors | (content_id, actor_id) | Ускорение JOIN-запросов между контентом и актерами. |
| content_genre | (content_id, genre_id) | Ускорение JOIN-запросов между контентом и жанрами. |
| genre | name | Быстрый поиск жанров по названию. |
| actor | name | Поиск актеров по имени. |
| favorite | user_id | Поиск избранного по пользователю. |
| watch_history | user_id, media_id, content_title | Аналитика просмотров по пользователю, медиа и названию контента. |
| recommendation | user_id | Персонализированный доступ к рекомендациям. |
| notification | user_id, delivery_status, sent_at | Управление уведомлениями по статусу и времени отправки. |

### Выбор СУБД (потаблично)
| Таблицы | СУБД | Обоснование |
| --- | --- | --- |
| user | **MySQL** | Необходимо обеспечение надежности и безопасности. ACID-транзакции для управления подписками и профилями. Поддерживает синхронную репликация (master-master) для гарантии сохранности данных. Хорошо подходит для частых операций записи. |
| media, track, component, event | **Cassandra** | Является частью системы **NMDB** (Netflix Media Data Base). Подходит для хранения больших объемов данных с частыми операциями чтения и записи. Optional column sets подходят для хранени таблиц со сложной структурой, например, таблицы component, поля которой будет различаться в зависимоти от типа media. **ElasticSearch** хранит индексы наложенные на поля этих таблиц, а также связи event и componentс родительскими элементом track. Для раздачи горичих данных из Cassandra используется **EVCache**. |
| poster, content, movie, series, episode, trailer, content_actors, content_genre, genre, actor | **PostgreSQL** | Данные таблички содержат структурированные данные и являются важными с точки зрения бизнес-логики, потому к ним применяются высокие требования к консистентности и надежноcти хранения. Редко изменяются, чаще используются для чтения. Поддерживает jsonb поля (которые используются в таблице content). Оптимизация обеспечивается за счет индексации в поисковой системе **ElasticSearch** (для синхронизации данных используется **Delta**, построенный на основе **Kafka Keystone**) и использования **EVCache** |
...
- метаданные о медиа хранятся в **Cassandra**, перед ней находится **MDPS** (Media Data Persistence Service), который подтвержает данный по принципу local quorum (подтверждение в пределах ДЦ -> минимизированы задержки, по соблюдается принцип read-after-write)
...
| track | media_id | Дорожки связаны с медиа, обеспечивают локальность данных. |
| component | track_id | Компоненты связаны с дорожками, обеспечивают локальность данных. |
| event | component_id | События связаны с компонентами, обеспечивают локальность данных. |
| favorite, watch_history, recommendation, notification | user_id | Данные связаны с пользователем, обеспечивают локальность данных. |
...
#### ElasticSearch
***шардирование***
- шардирование для всех таблиц будем проводить на основе первичных ключей 
...
| movie, series, episode, trailer, poster | content_id | Фильмы, сериалы, эпизоды, трейлеры и постеры, связаны с контентом и друг с другом. Обеспечивается локальность данных. |
...
- для контроля пуля соединений и балансировки запросов MySQL - ProxySQL, PostgreSQL - PgBouncer
...
- **PostgreSQL/MySQL**: WAL + логические дампы через pg_dump/mysqldump
