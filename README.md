...
- чтение из таблицы watch_history - 2465 (сравнимо с обращением к таблице media) * 3 (будем считать, что примерно столько раз может прерываться каждый пользовательский сеанс, чтобы стало необходимо узнать его прогресс просмотра) - 7395, запись сравнима с обращением к сегментам таблицы event (сохраняем прогресс просмотра фильма), но будем считать, что частота обращений это не раз в 4 секунды, а раз в 20 секунд, что в 5 раз меньше (1_912_500 / 5) - 382_500
...
Итого: 102 байт * 3_711_825_000 ≈ 352.6 Гб
...
#### Auth Service
- осуществляет аутентификацию через сессии с использованием Session ID
- генерирует уникальный идентификатор сессии, сохраняет данные в Tarantool
- передает Session ID клиенту через защищенные cookie/HTTP-заголовки
- проверяет валидность сессий через Zuul перед маршрутизацией запросов
- для ускорения проверки активных сессий используется EVCache
...
- При расчете нагрузки на сервис будем использовать отношение RPS на данный сервис к общему количеству [ЦОД](#расположение-дц) - 19.

- За основу взяты расчеты [RPS по всем типам запросов](#rps-в-разбивке-по-типам-запросов) и [сетевого трафика](#сетевой-трафик) 

- **Zuul**
    - Рассчитаем суммарный пиковый RPS, иcходя из RPS по всем типам запросов - 4_624_172, итого на Zuul в пределах одного ДЦ приходится 4_624_172 / 19 = 243_377 RPS
    - Аналогично для Net - 136_932 / 19 = 7_207 Гбит/c.

- **User Service**
    - RPS - 0.1 * 91.8M (DAU) / 3600 / 19 = 134 (аналогично расчету пикового [RPS по типам запросов](#rps-в-разбивке-по-типам-запросов))
    - Net - 91.8M * 1Кб / (1024 * 1024) / 19  = 5 Гбайт/сутки = 0.0005 Гбит/c.

- **Media Service** и **Streaming Service**
    - RPS - сумма пикового RPS для разного качества видео - 4_590_000 / 19 = 241_579 RPS
    - аналогично для Net - 127_296 / 19 = 6700 Гбит/с.

- **Recomendation Service**
    - RPS - RPS [обращений к табличке watch_history](#расчет-нагрузки-на-чтение-и-запись) - (7395 + 382_500) * 2 (пиковый) / 19 =  41_041 RPS
    - Net - 10.9 / 19 = 0.574 Гбит/сек

- **Search Service**
    - RPS - 6375 / 19 = 336 RPS
    - Net - 49.8 / 19 = 2.62 Гбит/сек
...
| Сервис | Технология | Характер сервиса | Целевой RPS | CPU (ядра) | RAM | Net (Гбит/c) |
| --- | --- | --- | --- | --- | --- | --- |
| Zuul | Go (Gin) | Маршрутизация (L7-балансировка) | 243_377 | 49 | 2.45 ГБ | 7_207 |
| User Service | Java (Spring) | Тяжелая бизнес-логика | 134 | 1 | 0.27 ГБ | 0.0005 |
| Media Service | Java (Spring) | CRUD API (метаданные) | 241_579 | 483 | 144.9 ГБ | 6_700 |
| Streaming Service | Go | Потоковая передача данных | 241_579 | 81 | 8.1 ГБ | 6_700 |
| Recommendation | Python + Java | ML-инференс + CQRS | 41_041 | 4_104 | 8.2 ТБ | 0.574 |
| Search Service | Java (Elastic) | Поисковые запросы | 336 | 1 | 0.34 ГБ | 2.62 |
| Content Service | Node.js | Легкое JSON API | 940 | 5 | 1.41 ГБ | 73.84 |

### БД

- User Service **Tarantool** 
    - 134 RPS
    - ROM ([размер хранения пользовательских данных](#размер-хранения)): 1_328 Tб * 20% = 2_656 Гб (1_328 Тб - это информация о пользователях за все время, будем считать, что реально активных пользователей - 20%)
- Media Service **Elastic Search** 
    - 241_579 RPS
    - ROM (es_media_chunk, es_media_event из [расчета размеров хранения для ES](#elasticsearch)): 352.6 Гб + 55 ГБ = 408 Гб
- Streaming Service **S3** 
    - 241_579 RPS
    - ROM ([размер хранения](#размер-хранения) видео и сериалов): 31_792 Тб + 109_002 Тб + 239.94 Тб + 822.66 Тб =  141_857 Тб = 139 Пб
- Recommendation Service **Cassandra** 
    - 41_041 RPS
    - ROM (watch_history, favourite, recomendation из [расчета размеров данных](#размеры-данных)): 240.8 Тб + 745 Гб + 55.9 Тб = 242 Тб
...
| --- | --- | --- | --- |
| Tarantool | In-Memory, высоконагруженные транзакции | 5_000 | 512 МБ |
| Elasticsearch | Поиск и индексация в реальном времени | 1_000 | 128 МБ |
| S3 | Объектное хранилище, высокая емкость | 500 | 32 МБ |
| Cassandra | Распределенные записи, высокая доступность | 2_000 | 64 МБ |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| User Service | Tarantool | 134 | - | 1 | 512 МБ + 2_656 ГБ | - | 84 (42 × 2) | 2_657 ГБ / 64 ГБ (RAM на сервер) = 42 сервера. Резервирование 2x. |
| Media Service | Elasticsearch | 241_579 | 408 ГБ | 242 | 30.9 ГБ | 2 ТБ | 8 (4 × 2) | 408 ГБ / 128 ГБ = 3.1 → 4 сервера. Резервирование 2x. |
| Streaming Service | S3 | 241_579 | 139 ПБ | 484 | 15.5 ГБ  | 100 ТБ | 2_846 (1_423 × 2) | 139 ПБ / 100 ТБ = 1_423 сервера. Резервирование 2x. |
| Recommendation | Cassandra | 41_041 | 242 ТБ | 21 | 1.3 ГБ | 16 ТБ | 38 (19 × 2) | 242 ТБ / 16 ТБ = 15.1 → 16 серверов. Резервирование 2x. |
| Content Service | PostgreSQL | 940 | 6.3 ТБ | 4 | 256 МБ | 2 ТБ | 8 (4 × 2) | 6.3 ТБ / 2 ТБ = 3.15 → 4 сервера. Резервирование 2x. |

### Конфигурация для Kubernetes-сервисов

| Сервис | CPU/r | CPU/l | RAM/r | RAM/l | Cnt |
| --- | --- | --- | --- | --- | --- |
| Zuul | 1 | 2 | 0.5 ГБ | 1 ГБ | 50 |
| Media Service | 8 | 16 | 4 ГБ | 8 ГБ | 60 |
| Content Service | 1 | 2 | 0.3 ГБ | 0.6 ГБ | 5 |
...
| --- | --- | --- | --- | --- | --- | --- |
| zuul-node | AWS | 2xEPYC 7B12/32x64GB/4xNVMe8T/4x400Gb/s | 128 | 36 | $5,400 | CPU: 128 × 18 = 2,304 ≥ 49.<br> RAM: 64 ГБ × 18 = 1,152 ГБ ≥ 2.45 ГБ.<br> Net: 400 Гбит/c × 36 = 14,400 Гбит/c ≥ 7,207. |
| user-svc-node | AWS | 2x4310/8x64GB/2xNVMe2T/2x10Gb/s | 32 | 2 | $120 | CPU: 32 × 1 = 32 ≥ 1.<br> RAM: 64 ГБ × 1 = 64 ГБ ≥ 0.27 ГБ.<br> Net: 10 Гбит/c × 2 = 20 Гбит/c ≥ 0.0005. |
| media-svc-node | AWS | 2xEPYC 7B12/32x128GB/4xNVMe8T/4x400Gb/s | 128 | 36 | $10,800 | CPU: 128 × 18 = 2,304 ≥ 483.<br> RAM: 128 ГБ × 18 = 2,304 ГБ ≥ 144.9 ГБ.<br> Net: 400 Гбит/c × 36 = 14,400 Гбит/c ≥ 6,700. |
| stream-svc-node | AWS | 2xEPYC 7B12/32x128GB/4xNVMe8T/4x400Gb/s | 128 | 36 | $10,800 | CPU: 128 × 18 = 2,304 ≥ 81.<br> RAM: 128 ГБ × 18 = 2,304 ГБ ≥ 8.1 ГБ.<br> Net: 400 Гбит/c × 36 = 14,400 Гбит/c ≥ 6,700. |
| recommend-node | AWS | 2xEPYC 7B12/64x256GB/8xNVMe16T/4x400Gb/s | 256 | 32 | $25,600 | CPU: 256 × 16 = 4,096 ≥ 4,104.<br> RAM: 256 ГБ × 16 = 4,096 ГБ ≥ 8.2 ТБ.<br> Net: 400 Гбит/c × 32 = 12,800 Гбит/c ≥ 0.574. |
| search-node | AWS | t4g.small/2x4GB/1xSSD100Gb/1x10Gb/s | 2 | 1 | $30 | CPU: 2 × 1 = 2 ≥ 1.<br> RAM: 4 ГБ × 1 = 4 ГБ ≥ 0.34 ГБ.<br> Net: 10 Гбит/c × 1 = 10 Гбит/c ≥ 2.62. |
| content-svc-node | AWS | 2x4310/8x64GB/2xNVMe2T/2x10Gb/s | 32 | 2 | $120 | CPU: 32 × 1 = 32 ≥ 5.<br> RAM: 64 ГБ × 1 = 64 ГБ ≥ 1.41 ГБ.<br> Net: 10 Гбит/c × 2 = 20 Гбит/c ≥ 73.84. |

#### Базы данных

| Название | Хостинг | Конфигурация | Cores | Cnt | Аренда/мес | Обоснование |
| --- | --- | --- | --- | --- | --- | --- |
| user-db-node | AWS | 2xEPYC 7B12/32x128GB/4xNVMe8T/4x400Gb/s | 128 | 84 | $10,800 | CPU: 128 × 42 = 5,376 ≥ 1.<br> RAM: 128 ГБ × 42 = 5,376 ГБ ≥ 2,656 ГБ.<br> ROM: Tarantool (in-memory). |
...
| postgres-node | AWS | Managed (16 vCPU, 128 ГБ RAM) | 16 | 8 | $9,600 | CPU: 16 × 4 = 64 ≥ 4.<br> RAM: 128 ГБ × 4 = 512 ГБ ≥ 256 МБ.<br> ROM: 2 ТБ × 4 = 8 ТБ ≥ 6.3 ТБ. |
